(function() {
  angular.module('Data', []).factory('Data', [
    'ngProgressFactory',
    '$rootScope',
    '$q',
    function(ngProgressFactory, $rootScope, $q) {
      ...
      Data.concatExtraColsWeb = function(row) {
        ...
        for (var key in row) {
          ...
          if (key == 'trace') row.exception = row[key];
          else if (
      ...
      Data.parseWebXMLLog = function(logContents) {
        ...
        if (!json) {
          ...
          if (!json) {
            ...
            Data.logParserError(
              'Unhandled & unparsable web log found, check the javascript console for details.',
              'Warning',
              ''
            );
            ...
            if (
              $rootScope.dataset.state.filesParsed ==
              $rootScope.dataset.state.filesParsing
            ) {
        ...
        console.log(
          'current weblog dataset contains this many log rows: ',
      ...
      Data.processKernelMessage = function(message, dataset, index) {
        ...
        if (message.length > 2) {
          for (var i = 2; i < message.length; i++) command += message[i];
        }
        ...
        if (command.startsWith('<st')) {
          ...
          command.startsWith(
            '<'
          ) /*|| command.startsWith("<mi") || command.startsWith("<svrd")*/
        ) {
          // xml command
          var xml = '<xml>' + command + '</xml>';
          var x2js = new X2JS();
        ...
        var logContext = context[2].substr(
          0,
          context[2].lastIndexOf('XML') - 1
        ); //[HOST:SERVERNAME][PID:5960][THR:6368][Kernel XML API][Trace]
      ...
      Data.parseKernelXMLLog = function(logContents) {
          ...
          for (var i = 0; i < messages.length; i++) {
            ...
            if (splitMessage.length < 2) {
              ...
              if (message.indexOf('ReturnCode') !== -1) {
                ...
                message.indexOf('# MicroStrategy Log version') !== -1
              ) {
                // skip lines that don't have a log line, e.g. "# MicroStrategy Log version 2.0"
                console.log(
                  'Silently skipping this message, since it has no value: ' +
                    message
                );
                continue;
              } else if (message.length == 0) {
                console.log(
                  'Silently skipping empty log line, since it has no value: ' +
                    message
                );
                continue;
              } else {
                console.error(
                  'Unhandled & unparsable log line found ',
                  message
                );
                Data.logParserError(
                  'Unhandled & unparsable log line found, check the javascript console for details. Last skipped log message: ' +
                    message,
                  'Warning',
                  message
                );
        ...
        console.log(
          'current kernelAPI dataset contains this many log rows: ',
          $rootScope.dataset.logs.kernel.length
        );
      };

      Data.recogniseLog = function(logContents) {
        if (logContents.startsWith('<record')) return 'webXML';
      ...
      Data.logParserError = function(error, level, logLine) {
        ...
        var error = {
          data: {
            status: level,
            message:
              $rootScope.dataset.indexes.skipped + ' lines were not parsed',
            cause: 'Reason: ' + error
          }
        };
      ...
      Data.handleError = function(e) {
        if (typeof e.data != 'undefined' && e.data) {
          ...
          console.error(
            e.data.status + ' : ' + e.data.message + ' : ' + e.data.cause
          );
